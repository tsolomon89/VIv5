...
export const AppShell: React.FC<AppShellProps> = ({ children, sidebarContent, isSidebarOpen, setScrollContainer }) => {
    return (
        <div className="relative h-screen w-full overflow-hidden bg-[#1A1A1B]">
            ...
            {/* Sidebar - Fixed Right */}
            <div 
                className={`fixed top-0 right-0 h-screen w-[420px] bg-[#111] border-l border-white/10 z-40 shadow-2xl transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}
            >
